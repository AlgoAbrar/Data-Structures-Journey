...
// Function to delete the root element of the max-heap
void delete_heap(vector<int> &v)
{
    ...
    while(true)
    {
        ...
        // If left child is larger and greater than the current node, swap
        if(left_val >= right_val && left_val > v[cur_idx])
        {
            swap(v[left_idx], v[cur_idx]);
            cur_idx = left_idx;
        }
        // If right child is larger and greater than the current node, swap
...
Algorithm:
1. **insert_heap**: Insert a new value into the max-heap.
   - Push the new value to the back of the vector.
   - Move the new value up to maintain the max-heap property by comparing it with its parent and swapping if necessary.
   
2. **delete_heap**: Delete the root (maximum) element from the heap.
   - Replace the root with the last element.
   - Reorganize the heap by moving the root down to maintain the max-heap property, comparing it with its children and swapping when necessary.
