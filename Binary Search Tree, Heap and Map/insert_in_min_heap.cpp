...
int main()
{
    ...
    while(cur_idx != 0)
    {
        ...
        if(v[par_idx] > v[cur_idx]) // If the current node is smaller than the parent, swap them
...
3. Read an integer 'val' to be inserted into the min-heap.
4. Add the value 'val' to the end of the vector (representing a min-heap).
5. Perform the "heapify-up" process to maintain the min-heap property.
6. Print the elements of the heap.

Time Complexity:
- Reading 'n' integers takes O(n).
- Inserting an element into a min-heap takes O(log n) (heapify-up operation).
